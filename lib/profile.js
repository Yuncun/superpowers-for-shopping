...
function parseTable(body, heading) {
  ...
  return m[2].split('\n')
    ...
    .map(line => {
      // Tolerate rows with or without a trailing pipe. `| a | b | c |` splits
      // to ['', 'a', 'b', 'c', ''] while `| a | b | c` splits to
      // ['', 'a', 'b', 'c']. Drop the empty leading/trailing cells only.
...
function sanitizeCell(value) {
  // Replace pipe with slash so table column count stays intact, and collapse
  // any CR/LF to a single space so one logical row doesn't split into many.
...
/**
 * Returns all purchase_history rows where kept === '?'.
 * Row shape: { date, item, brand, '$': price, kept, notes }
 */
export async function listPendingPurchases() {
  const p = await readProfile();
  return p.purchase_history.filter(r => r.kept === '?');
}

/**
 * Finds the first purchase_history row matching (date, item, brand) exactly.
 * - If not found: returns { updated: false, reason: 'not_found' }.
 * - If kept is not 'yes' or 'no': throws Error with .code = 'invalid_kept'.
 * - Otherwise: mutates kept (and notes if provided), writes profile, returns { updated: true }.
 * Tiebreaker: first match wins (index order in purchase_history).
 */
export async function updatePurchase({ date, item, brand }, { kept, notes } = {}) {
  if (kept !== 'yes' && kept !== 'no') {
    const err = new Error('invalid_kept');
    err.code = 'invalid_kept';
    throw err;
  }
  const p = await readProfile();
  const row = p.purchase_history.find(
    r => r.date === date && r.item === item && r.brand === brand,
  );
