...
async function runAgentBrowser(args, { execImpl = defaultExec } = {}) {
  let fullArgs = args.includes('--profile')
    ? args
    : ['--profile', browserProfilePath(), ...args];
  if (!fullArgs.includes('--json')) fullArgs = [...fullArgs, '--json'];
...
export async function getCookieHeader(host, { execImpl } = {}) {
  const normalizedHost = normalizeHost(host);
  const data = await runAgentBrowser(['cookies', 'get'], { execImpl });
...
// Best-effort heuristic — true if ANY cookie exists for the host. Anonymous
// visits set cookies too, so this can return true for non-logged-in sessions.
// The authoritative check is Plan 5 calling addToCart and getting back
// {ok: false, error: 'authentication_required'} when the session is actually expired.
export async function isLoggedIn(host, opts = {}) {
  const cookie = await getCookieHeader(host, opts);
