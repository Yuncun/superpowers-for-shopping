...
// Gender tokens recognized in Shopify URL slugs. Index in the array isn't
// meaningful — these are just words that gate-keep gender categories.
const GENDER_SLUG_TOKENS = ['mens', 'womens', 'men', 'women', 'kids'];

/**
 * Prepend a gender word to the search query so Shopify suggest favors that
 * category. Empty `shoppingFor` returns the query unchanged.
 */
export function prefixForShoppingFor(searchQuery, shoppingFor) {
  const pref = String(shoppingFor ?? '').trim().toLowerCase();
  if (!pref) return searchQuery;
  // Avoid double-prefixing if user already typed it.
  if (searchQuery.toLowerCase().startsWith(pref)) return searchQuery;
  return `${pref} ${searchQuery}`;
}

/**
 * Return true if a product URL's pathname contains a gender slug that
 * contradicts `shoppingFor`. Used to drop e.g. /products/womens-* listings
 * when the user is shopping for men.
 */
export function urlContradictsShoppingFor(url, shoppingFor) {
  const pref = String(shoppingFor ?? '').trim().toLowerCase();
  if (!pref) return false;
  let path;
  try { path = new URL(url).pathname.toLowerCase(); }
  catch { return false; }
  const myToken = pref.replace(/s$/, ''); // 'mens' → 'men' for matching purposes
  for (const token of GENDER_SLUG_TOKENS) {
    const norm = token.replace(/s$/, '');
    if (norm === myToken) continue;
    // word-boundary match anywhere in the path
...
export async function runCartFlow({
  ...
  // `searchQuery` (the simplified noun phrase, optionally gender-prefixed).
  const simplified = simplifyQuery(query) || String(query ?? '').trim();
  const searchQuery = prefixForShoppingFor(simplified, profile.shopping_for);
  ...
  try {
    ...
    // Strategy when shopping_for is set: try the prefixed query first (best
    // precision on retailers that include gender words in product titles, e.g.
    // Marine Layer). If a retailer returns zero hits for the prefixed query,
    // retry with the plain query — some retailers (e.g. Everlane) only mark
    // gender in URL slugs, not titles, so the prefix kills recall. The URL
    // slug filter downstream cleans up cross-gender bleed in the plain case.
    const prefixed = searchQuery !== simplified;
    const settled = await Promise.all(
      resolvedRetailers.map(async (host, i) => {
        try {
          let results = await search(host, searchQuery);
          if (results.length === 0 && prefixed) {
            results = await search(host, simplified);
          }
    ...
    // Also drop products whose URL contains a gender slug that contradicts
    // profile.shopping_for (e.g. /products/womens-* when shopping_for='mens').
    ...
    for (let i = 0; i < resolvedRetailers.length; i++) {
      ...
      for (const c of ranked) {
        ...
        if (urlContradictsShoppingFor(c.url, profile.shopping_for)) continue;
