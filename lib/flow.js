...
export async function runCartFlow({
  ...
  try {
    ...
    // Shopify cart permalink: visiting this URL in the user's browser adds the
    // variant to THEIR cart (using their own cookies), then redirects to /cart.
    // Without this, the Node-side addToCart added to a discarded cookie jar and
    // the user's browser saw an empty cart.
    const cartUrl = `https://${host}/cart/${top.variants[0].variant_id}:1`;
    session.pushState({ stage: 'redirect', url: cartUrl });
    await sleep(2000);
    return { outcome: 'success', product: top, cartUrl };
  } catch (err) {
    // Idle cleanup or manual close rejects pending waiters with session_closed.
    // Treat it the same as the user dismissing the tab.
